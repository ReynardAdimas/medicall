...
//     final stok = int.tryParse(stokController.text.trim()) ?? 0; // Pastikan stok adalah angka, default 0
//
//     if (nama.isEmpty) {
...
//     if (stok <= 0) {
//       ScaffoldMessenger.of(context).showSnackBar(
//           const SnackBar(content: Text('Stok Obat harus lebih dari 0!'))
//       );
//       return;
//     }
//
//     // masukin yang mau disimpen ke variabel
//     final newObat = Obat(nama: nama, stok: stok);
//
//     try {
//       // save in db
//       await obatDatabase.createObat(newObat); // Gunakan await karena createObat mungkin async
//       ScaffoldMessenger.of(context).showSnackBar(
//           const SnackBar(content: Text('Obat berhasil disimpan!'))
//       );
//       Navigator.pop(context, true); // Kembali ke halaman sebelumnya
//     } catch (e) {
//       print('Error saat menyimpan obat: $e');
//       ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(content: Text('Gagal menyimpan obat: ${e.toString()}'))
//       );
//     }
//   }
//
...
class _Obat2State extends State<Obat2> {
  ...
  void saveObat() async {
    ...
    try {
      // 1. Cek apakah obat dengan nama yang sama sudah ada
      final Obat? existingObat = await obatDatabase.getObatByName(nama);

      if (existingObat != null) {
        // 2. Jika obat sudah ada, update stoknya
        ...
        // 3. Jika obat belum ada, buat entri baru
